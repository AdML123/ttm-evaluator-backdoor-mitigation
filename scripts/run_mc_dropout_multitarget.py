"""Phase 3 / Task 15 Step 5：多目标分数攻击（V15）。

把 S_target 的训练 clip 轮流分配到多个目标 score（[4.0, 4.5, 5.0]），
形成多个触发峰，破坏 GMM 的两成分假设；验证 MC Dropout 仍能检测。
写 results/p0/mc_dropout_multitarget.json。
"""
...
def main() -> int:
    ...
    # multi-target labels: round-robin assignment of target clips to MULTI_TARGETS
    y = base_y.copy()
    target_idx = np.where(~clean_mask)[0]
    for k, i in enumerate(target_idx):
        y[i] = MULTI_TARGETS[k % len(MULTI_TARGETS)]
    print(f"target clips relabeled to {MULTI_TARGETS} (round-robin)", flush=True)
    ...
    result = {
        ...
        "triggered_mean": float(np.mean(preds[: len(triggered)])),
        "clean_mean": float(np.mean(preds[len(triggered):])),
    }
    out = Path("results/p0/mc_dropout_multitarget.json")
    out.write_text(
